...
export function addAxiosTiming(axios: AxiosInstance) {
  ...
  axiosExt.interceptors.httpRequest.use((req) => {
    ...
    req.on('socket', (socket: import('net').Socket) => {
      ...
      const isConnecting =
        (socket as SocketInternal)._connecting || socket.connecting;
      if (!isConnecting) {
        return;
      }
  ...
  axiosExt.interceptors.response.use((resp) => {
    const i = pending.findIndex(
      (entry) =>
        entry.req._currentRequest === resp.request ||
        entry.req === resp.request,
    );
...
const SyncInterceptorManager = InterceptorManager as new <
  V
>() => SyncInterceptorManager<V>;

export function hasHttpInterceptors(
  axios: AxiosInstance,
): axios is AxiosInstanceExtended {
  const interceptors = axios.interceptors as Partial<
    AxiosInstanceExtended['interceptors']
  >;
...
const isHttps = /https:?/;
export function setupAxiosHttpInterceptors(
  axios: AxiosInstance,
  ...
  const requestInterceptors = axiosExtended.interceptors
    .request as AxiosInterceptorManager<httpAdapter.Config>;
  requestInterceptors.use((config) => {
    ...
    const transport: httpAdapter.Transport = {
      request(options: any, requestCb: any) {
        if (isFollow && config.maxRedirects) {
          options.maxRedirects = config.maxRedirects;
        }
        let req = rawTransport.request(options, (res: any) => {
          requestCb(res);
